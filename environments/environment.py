...
class OthelloEnv:
    ...
    def __init__(self, board_size: int = 5):
        if board_size < 4:
            raise ValueError("board_size musí být alespoň 4.")
    ...
    def step(self, action: int):
        ...
        if action not in legal_actions:
            # The task specification says an illegal move should mean automatic loss.
            self.done = True
            info = {
                "illegal_move": True,
                "winner": -self.current_player,
                "disc_diff": self.get_disc_difference(self.current_player),
            }
            reward = -1
        ...
        if action == self.pass_action:
            # Pass is legal only when there is no other move.
            pass
        else:
        ...
        # Check for game end:
        # the game ends when neither player has a normal move.

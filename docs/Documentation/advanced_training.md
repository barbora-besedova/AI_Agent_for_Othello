...
```bash
# New experiment (auto-increments: experiments_001, experiments_002, ...)
...
wsl.exe pkill -f advanced_training

# Start a fresh run
wsl.exe bash -c "... .venv/bin/activate && cd /home/cube/projects/AI_Agent_for_Othello/all_in_clean && python advanced_training.py --load_model_path models/guided_per_dqn_6_best_overnight.pth"
The new run will create Training/experiments_001/ (or _002 if _001 already exists).
...
└── checkpoints/
    ├── latest.pth           ← most recent model (overwritten each save)
    ├── model_ep1000.pth     ← preserved copies at each eval interval
    ├── latest_train.csv     ← training metrics (loss, Q, grad norm, etc.)
...
| `checkpoints/latest.pth` | Current model (overwrites) | Every save interval |
| `checkpoints/model_epN.pth` | Historical checkpoint at episode N | Every save interval |
| `checkpoints/latest_train.csv` | Training metrics over time | Incremented each print |
| `checkpoints/latest_eval_epN.csv` | Per-opponent eval at episode N | Every eval interval |
| `checkpoints/games/*.json` | Full move-by-move game transcripts | At specified episodes |
...
- Creates `experiments_001/` (or `_002`, `_003`, ... auto-incremented)
...
```bash
uv run python advanced_training.py \
    --experiment_dir experiments_001
```

- Reads `resume_state.json` to get last completed episode + epsilon
- Loads the checkpoint from `checkpoints/model_ep{N}.pth`
...
3. **Checkpointing** (every `save_every` / `eval_every` episodes)
   - Saves model to `checkpoints/latest.pth`
   - Copies to `checkpoints/model_ep{N}.pth` (preserved)
...
4. **Shutdown**
   ...
   - On manual kill: same — resume_state.json has the last save point
...
checkpoints/model_ep{N}.pth   → model weights + optimizer state + config
setup.json                    → original training parameters
```

On resume:
1. Read `resume_state.json` → get `episode`, `epsilon`
2. Load model from `checkpoints/model_ep{episode}.pth`
...
```bash
cat experiments_001/setup.json
cat experiments_002/setup.json
```
...
The CSV metrics (`latest_train.csv`, `latest_eval_ep*.csv`) contain the
actual performance data for plotting.
